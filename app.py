...
# Streamlit Community Cloud exposes values from secrets.toml through
# ``st.secrets`` rather than os.environ.  Keep .env working locally, while
# accepting the same names from Streamlit and Render secret settings.
try:
    streamlit_secrets = st.secrets.to_dict()
except FileNotFoundError:
    streamlit_secrets = {}
for setting in ("GROQ_API_KEY", "GROQ_MODEL", "GROQ_STT_MODEL", "GROQ_MAX_TOKENS"):
    if not os.getenv(setting) and setting in streamlit_secrets:
        os.environ[setting] = str(streamlit_secrets[setting])
...
if run_btn:
    if not uploaded_file:
        ...
        if missing_keys:
            st.error("Missing required setting(s) in .env: " + ", ".join(missing_keys))
            st.stop()
        ...
        try:
            ...
            chunks = []
            if not transcript:
                raise RuntimeError("No speech was detected in the supplied audio.")
            ...
            cleanup_audio_files(chunks)
            cleanup_temporary_source(input_source)
